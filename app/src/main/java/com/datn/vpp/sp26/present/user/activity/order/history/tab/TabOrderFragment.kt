...
import com.datn.vpp.sp26.present.user.activity.order.history.InvoiceViewActivity
...
class TabOrderFragment : BaseFragment<FragmentTabOrderBinding>() {
    ...
    private fun openInvoiceView(order: ResOrderDTO) {
        startActivity(
            Intent(requireContext(), InvoiceViewActivity::class.java).apply {
                putExtra(AppConst.KEY_ORDER_INVOICE, Gson().toJson(order))
            }
        )
    }
